<template>
  <div>
    ...
    <p>{{ disclaimerText2 }}
    <h1>{{ licenseTitel }}</h1>
    <p>{{ licenseText1 }}<a @click="link('https://github.com/BrickTwo/BrickHunter/blob/master/LICENSE')" href="">{{ licenseLink }}</a>{{ licenseText2 }}</p>
    <p>{{ sourceCodeText1 }}<a @click="link('https://github.com/BrickTwo/BrickHunter')" href="">{{ sourceCodeLink }}</a>{{ sourceCodeText2 }}</p>
  </div>
</template>
