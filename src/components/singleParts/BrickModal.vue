<template>
  ...
  >
    <b-nav tabs>
      ...
      <b-nav-item :active="page == 'chart'" @click="page = 'chart'" disabled>
        {{ labelChart }}
      </b-nav-item>
    </b-nav>
    <b-container v-if="page == 'data'">
      ...
      <b-row class="p-1 mt-0">
        ...
        <b-col v-else cols="4" class="p-0 text-right"> - </b-col>
      </b-row>
      <b-row class="p-1 mt-0 stripe">
      ...
      <b-row class="p-1 mt-0">
        ...
        <b-col v-else cols="7" class="p-0 text-right"> - </b-col>
      </b-row>
      <b-row class="p-1 mt-0 stripe">
        ...
        <b-col
          v-if="
            brick.lastSeen &&
            brick.lastSeen.substr(brick.lastSeen.length - 1) == 'Z'
          "
          cols="7"
          class="p-0 text-right"
        >
...
export default {
  ...
  methods: {
    async loadBrick() {
      ...
      // if (response.maxAmount) this.prepareMaxAmountDataset(response.maxAmount);
  ...
  beforeMount() {
    this.colors = this.COLOR.filter((c) => c.legoId == this.brick.color).sort(
      (a, b) => {
