  ...
  <b-col cols="3" class="p-1 h-100">
    <b-container class="brick p-2">
      ...
      >
        ...
        <div
          ...
          v-if="$store.state.singleParts.notificationChatId"
        >
          <font-awesome-icon
          ...
          >
            <b-form-checkbox
              ...
              @change="editNotificationItemNumber(brick.itemNumber)"
            >
              {{ labelElement }}
            </b-form-checkbox>
            <b-form-checkbox
      ...
      <b-row
        v-if="brick.deliveryChannel == 'pab'"
        style="
          background-color: #dbd935;
          color: white;
          border-radius: 0.25rem;
          margin: 0 -5px;
          padding: 0 5px;
      ...
      <b-row
        v-if="brick.deliveryChannel == 'oos'"
        style="
      ...
      <b-row v-if="brick.country">
        <b-col>
          <b-img :src="getFlagImgUrl(brick.country)" width="20px" />
        </b-col>
      ...
      <b-row v-else>
        <b-col class="text-right"> {{ labelNoPrice }} </b-col>
      </b-row>
      <!-- </b-overlay> -->
      <b-row
        style="
      ...
      >
        ...
        <span style="white-space: nowrap">{{ color.brickLinkName }}</span>
      </b-row>
    </b-container>
    <b-button
      v-if="order <= 0"
    ...
    <b-container v-else class="pos" style="margin-top: -5px">
      <b-row>
        ...
        <b-col class="m-0 p-0 text-center" style="max-width: 5%"> </b-col>
        <b-col
          class="m-0 p-0 text-right"
...
export default {
  ...
  methods: {
    ...
    getFlagImgUrl(value) {
      if (!value) return "flags/trans.png";
    ...
    addNotificationItemNumber(itemNumber) {
      this.$store.commit("singleParts/addNotificationItemNumber", itemNumber);
    },
    ...
    removeNotificationDesignId(designId) {
      this.$store.commit("singleParts/removeNotificationDesignId", designId);
    },
  ...
  beforeMount() {
    this.color = this.COLOR.find((c) => c.legoId == this.brick.color);
    if (!this.color || this.brick.color == null) {
  ...
  computed: {
    image() {
      ...
      return `https://brickhunter.blob.core.windows.net/parts/pab/${this.brick.itemNumber}.jpg`;
    },
    bgimage() {
      ...
      if (this.brick.imageUrl)
        return `background-image: url(${this.brick.imageUrl}), url('placeholder.jpg'); width: 100%; background-repeat: no-repeat; background-size: contain; background-position: center;`;
      return `background-image: url(https://brickhunter.blob.core.windows.net/parts/pab/${this.brick.itemNumber}.jpg), url('placeholder.jpg'); width: 100%; background-repeat: no-repeat; background-size: contain; background-position: center;`;
    },
    colorCode() {
    ...
    labelNoPrice() {
      return browser.i18n.getMessage("singlePartsNoPrice");
    },
    isAvailable() {
      console.log(
        this.brick.isAvailable,
        !parseInt(this.brick.isAvailable),
        this.brick.maxAmount,
        this.brick.priceAmount
      );
