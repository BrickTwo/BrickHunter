...
export default {
  ...
  methods: {
    ...
    async loadBricks(resetPage) {
      ...
      let colorLegoId = 99999;

      if (this.selectedColor != "all") {
        colorLegoId = this.COLOR.find((c) => c.id == this.selectedColor).legoId;
      }
      ...
      this.colorList = this.COLOR.filter((el) => {
        return this.search.colors.some((c) => {
          return el.legoId == c;
        });
      });
    ...
    async loadPrices() {
      ...
      this.search.bricks.forEach((brick) => {
        if (!designIds.find((d) => d == brick.designId) && brick.update) {
      ...
      for (var i = 0; i < designIds.length; i++) {
        ...
        if (!response) {
          var found = this.search.bricks.filter((b) => b.designId == designId);
            ...
            response.bricks.map((brick) => {
              var found = this.search.bricks.find(
                (b) =>
                  ...
                  b.alternativeItemNumbers.includes(`|${brick.itemNumber}|`)
              );
  ...
  created() {
    ...
    bus.$on(
      "showPartList",
      (partListId, showFavorites, showHaveIts, showNotifications) => {
        this.$store.commit("addLog", {
          action: "showPartList",
        });
  ...
  computed: {
    ...
    labelSinglePartsInfo() {
      return browser.i18n.getMessage("singlePartsInfo");
    },
  },
};
</script>
