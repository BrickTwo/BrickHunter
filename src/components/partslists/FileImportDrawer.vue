  ...
  <n-drawer-content title="File Import" closable>
    <n-form :model="formValue" ref="formRef" :rules="rules">
      ...
      <n-space justify="start">
        <n-button type="primary" ghost @click="handleClickImport">
          <template #icon>
            <n-icon>
              <SaveAltOutlined />
            </n-icon>
...
export default defineComponent({
  ...
  setup() {
    onBeforeMount(async () => await partsListStore.init());
    const formValue = ref({
      name: "",
      textInpuit: "",
    ...
    return {
      ...
      handleClickImport() {
        ...
        partsListStore.importFromBrickLink(wantedList, formValue.value.name);
      },
      rules: {
        name: {
          required: true,
