...
const getters = {
  ...
  getPickABrickTotalPrice: (state) => () => {
    let price = 0;
    state.pickABrickList.map((pos) => {
      price += (pos.qty.order * pos.pickABrick.variant.price.centAmount) / 100;
    });
    return price;
  },
...
const mutations = {
  ...
  addToBricksAndPiecesList(state, payload) {
    var found = state.brickAndPiecesList.find(
      (f) => f.bricksAndPieces.itemNumber == payload.bricksAndPieces.itemNumber
    );
  ...
  addToPickABrickList(state, payload) {
    ...
    if (found) {
      ...
      if (
        payload.pickABrick.variant.attributes.maxOrderQuantity > 0 &&
        found.pickABrick.variant.attributes.maxOrderQuantity < payload.qty.order
      ) {
        payload.qty.order = found.qty.maxAmount;
      }
    } else {
      ...
      if (
        payload.pickABrick.variant.attributes.maxOrderQuantity > 0 &&
        payload.pickABrick.variant.attributes.maxOrderQuantity <
          payload.qty.order
      ) {
        payload.qty.maxAmount =
          payload.pickABrick.variant.attributes.maxOrderQuantity;
